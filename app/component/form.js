...
export default function Form() {
  ...
  const handleSubmit = (event) => {
    ...
    if (!teacherName || !section || !courseName || !price || !description || !status || !image) {
      setModalOpen(true);  // Open the modal if validation fails
      return;
    }

    // Handle form submission
    console.log('Form submitted:', { teacherName, section, courseName, price, description, status, image });
  };
  ...
  return (
    <>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="flex items-center justify-between mb-4">
            <button
              ...
              type="submit"
            >
              Submit <span className="px-2"><BiBadgeCheck size={20} /></span>
            </button>
          </div>
        </div>
      </form>
      <Modal 
        ...
        message="Please fill all the fields in order to add a course." 
      />
    </>
  );
}
